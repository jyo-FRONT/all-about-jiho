...
export const projectsData: ProjectItem[] = [
  ...
  {
    id: "ai-metrics-automation",
    companyLogo: inaviLogo,
    company: "아이나비시스템즈",
    tags: ["AI 자동화", "데이터 분석", "AARRR"],
    period: "2026.07 — 진행 중",
    title: "지표개선을 위한 업무 자동화 프로세스 구축",
    summary: "Admin·GA4·Jira·시장 데이터로 흩어져 있던 지표 분석을 AI로 자동화해, 데이터 수집부터 변화량 산출·원인 분석·인사이트 도출·개선 과제 초안 작성까지 매주 자동으로 실행되는 파이프라인을 설계했습니다. 사람은 데이터를 찾고 정리하는 대신 도출된 인사이트를 검토하고 최종 개선 과제를 판단하는 데 집중할 수 있게 되었습니다.",
    featured: true,
    coverImage: "/projects/ai-metrics-automation/cover.jpg",
    highlight: "Admin·GA4·Jira·시장 데이터 4종을 주 1회 자동 수집·분석 · Claude 기반 인사이트·개선 과제 초안 자동 생성",
    role: "데이터 수집 상세 플랜(기여도 100%), 실제 API 연결 및 스크립트 생성(기여도 80%), 오케스트레이션 설계(기여도 100%)",
    team: "PM(1), 기획(2) · 2026.07 — 진행 중",
    images: [
      "/projects/ai-metrics-automation/panel-1.jpg",
      "/projects/ai-metrics-automation/panel-2.jpg",
      "/projects/ai-metrics-automation/panel-3.jpg",
      "/projects/ai-metrics-automation/panel-4.jpg",
      "/projects/ai-metrics-automation/panel-5.jpg",
      "/projects/ai-metrics-automation/panel-6.jpg",
      "/projects/ai-metrics-automation/panel-7.jpg",
    ],
    process: [
      "배경 - 서비스는 오픈으로 끝나지 않고 지속적인 개선이 필요하지만, 서비스 데이터·사용자 행동 데이터·제품 변경 이력·시장 정보가 Admin·GA4·Jira·시장 리서치 등 각각 다른 도구에 흩어져 있고, GA4 같은 분석 도구도 매번 필요한 지표를 직접 탐색·조합해야 해 진입장벽이 높았음. 원인을 어떻게 해석하느냐에 따라 개인의 주관이 개입되는 문제도 있었음",
      "목표 - 데이터 수집부터 분석, 인사이트 도출, 개선 과제 초안 작성까지 반복되는 분석 프로세스를 AI로 자동화하고, 사람은 데이터를 찾고 정리하는 대신 도출된 인사이트를 검토해 최종 개선 과제를 판단하는 데 집중하도록 설계",
      "전략 - 데이터 수집·지표 계산 및 비교·인사이트 도출·개선 과제 초안까지 반복적이고 규칙적인 업무는 AI에게 위임하고, 비즈니스 우선순위 판단과 실행 여부 결정처럼 판단이 필요한 최종 개선 과제 선정만 사람이 개입하도록 역할을 분리",
      "오케스트레이션 설계 - Admin(회원·활동·문의)·GA4(유입·이벤트·퍼널)·Jira(배포·기능변경·이슈)·시장 리서치(경쟁사·스토어·뉴스) 4개 데이터 소스를 Trigger(주 1회 자동 실행) → Collect → Store(회차별 마크다운 저장) → Calculate(AARRR 지표 계산 및 직전 회차 비교) → Interpret(원인 분석 및 인사이트·최종 과제 도출) 하나의 파이프라인으로 연결, AARRR 리포트·인사이트 리포트·최종 과제 후보 3종을 산출",
      "실행 1: 수집 데이터 정의 - AARRR(Acquisition·Activation·Retention·Referral·Revenue) 관점에서 프로덕트별로 필요한 데이터와 소스를 정의 (예: Activation은 온보딩 완료율·가입→유효 액션 전환율을 GA4·어드민에서 수집)",
      "실행 2: 데이터 수집 자동화 - Claude Code로 Admin API와 GA4 API를 연동하는 수집 스크립트를 직접 제작해 데이터를 자동 수집하고, Jira는 아틀라시안 MCP로 연동해 배포·기능 변경 이력을 함께 수집, 결과는 AI가 읽기 쉬운 마크다운 형태로 저장",
      "실행 3: 변화량 산출 - 직전 회차 데이터를 기준선으로 설정하고 이번 회차와 비교해 AARRR 주요 지표의 변화량을 자동 산출하는 주간 리포트를 생성",
      "실행 4: 변화 원인 분석 및 인사이트 도출 - 지표에 유의미한 변화가 발견되면 Jira 배포 이력 등 내부 요인과 시장 변화 등 외부 요인으로 나누어 원인 후보를 확인하고, 제품 변경 이력과 시장 데이터를 교차 분석해 표면적 인과관계를 반박 검증하는 절차까지 포함해 근거 없는 해석을 걸러냄",
      "실행 5: 최종 개선 과제 도출 - Claude가 분석 결과를 바탕으로 개선 과제 후보와 우선순위를 제안하면, 사람이 비즈니스 우선순위를 판단해 최종 Top 3 과제를 선정",
      "회고 - 개발자가 아니어도 설정값 기반으로 반복 실행할 수 있는 운영성과, 서비스별 데이터·지표 정의만 분리하면 수집·분석·비교 프로세스는 재사용 가능한 확장성을 고려해 설계. 다만 초기 1주 단위 비교는 변화량이 충분히 누적되지 않아 회차 간 차이가 작게 나타날 수 있다는 한계를 확인",
      "향후 계획 - 서비스별 적정 분석 주기 검증, 단일 회차 비교에서 누적 추이 비교로 기준선 고도화, 동일한 분석 환경을 다른 서비스로 확장 적용 예정",
    ],
  },
  {
